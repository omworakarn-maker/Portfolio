...
export function Shell({ children, home = false }: { children: React.ReactNode, home?: boolean }) {
    ...
    return <div className={home ? "site-shell home-shell" : "site-shell"}>
        ...
        {panel && <div className="panel-wrap" onMouseDown={() => setPanel(null)}><aside onMouseDown={e => e.stopPropagation()}><button className="panel-close" onClick={() => setPanel(null)}>CLOSE ×</button>{panel === "resume" ? <><span className="micro">RESUME / CV</span><h2>Want a closer look?</h2><p>My resume will be available to download here soon. I’m currently preparing the latest version with my experience, skills, and selected projects.</p><a href="/resume.pdf" download className="capsule resume-download" style={{ textDecoration: 'none' }}>DOWNLOAD RESUME</a></> : <><span className="micro">WELCOME, CURIOUS ONE</span><h2>Come on in.</h2><label>Email<input type="email" placeholder="you@example.com" /></label><label>Password<input type="password" placeholder="••••••••" /></label><button className="capsule">Log in ↗</button></>}</aside></div>}
